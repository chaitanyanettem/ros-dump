...
ObstacleAvoidance::ObstacleAvoidance()
{
    ...
    //use seperate thread to handle window events returns 1 if possible to use another thread, 0 if not
...
void ObstacleAvoidance::imageProcessCB(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    if(threadSupported_ == 0)
        cv::waitKey(3); //program waits for 3 ms before continuing, supposed to be helpful for window interaction? not sure how it helps though.
}
